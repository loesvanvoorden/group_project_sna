...
def generate_voting_edgelist(data, period_name):
    ...
    if agreements_dict:
        ...
        for (party1, party2), counts in agreements_dict.items():
            # Store RAW agreement count (for analysis.Rmd normalization approach)
            # R will normalize by total motions later
            edgelist_data.append({
                'from': party1,
                'to': party2,
                'weight': counts['agreements']  # Raw count, not rate
            })
...
def load_and_process_coalition_data():
    ...
    print(f"  Loaded {len(coalition_edgelist)} coalition edges (all historical data)")
    return coalition_edgelist

def filter_coalition_by_period(coalition_edgelist, period="pre"):
    """
    Filter coalition data based on temporal validity and add historical coalitions for balance
    
    The 2024 coalition (BBB, NSC, PVV, VVD) was formed AFTER the 2023 election,
    so edges from this coalition should NOT appear in pre-election networks.
    
    To balance coalition counts between periods, we include the Kok II cabinet (1998-2002)
    in the pre-election data: PvdA-VVD-D66
    
    Args:
        coalition_edgelist: Full coalition edge list
        period: "pre" (before 2023 election) or "post" (after 2024 formation)
    
    Returns:
        Filtered coalition edge list
    """
    if period == "pre":
        # The 2024 coalition (formed July 2024) includes: BBB, NSC, PVV, VVD
        # These parties only coalitioned together in 2024 (weight=1), so exclude them
        parties_2024_coalition = {"BBB", "NSC", "PVV", "VVD"}
        
        # Exclude edges where:
        # - Both parties are from the 2024 coalition AND
        # - Weight is 1 (meaning they ONLY coalitioned in 2024, not historically)
        def is_2024_only_coalition(row):
            from_in = row['from'] in parties_2024_coalition
            to_in = row['to'] in parties_2024_coalition
            return from_in and to_in and row['weight'] == 1
        
        filtered = coalition_edgelist[~coalition_edgelist.apply(is_2024_only_coalition, axis=1)].copy()
        
        # Add Kok II cabinet (1998-2002) for temporal balance: PvdA-VVD-D66
        # This adds 3 edges to balance the 6 edges removed from the 2024 coalition
        kok_ii_edges = [
            ('D66', 'PvdA'),    # New edge (didn't exist in original data)
            ('D66', 'VVD'),     # +1 to existing weight
            ('PvdA', 'VVD')     # +1 to existing weight
        ]
        
        for party1, party2 in kok_ii_edges:
            ...
            existing = filtered[
                (filtered['from'] == party1) & (filtered['to'] == party2)
            ]
            
            if len(existing) > 0:
                # Increment existing weight by 1
                filtered.loc[
                    (filtered['from'] == party1) & (filtered['to'] == party2),
                    'weight'
                ] += 1
                ...
                filtered = pd.concat([filtered, new_edge], ignore_index=True)
        
        print(f"  Pre-election: Filtered {len(coalition_edgelist)} -> {len(filtered)} edges")
        print(f"    • Excluded 2024 coalition (BBB/NSC/PVV/VVD)")
        print(f"    • Added Kok II cabinet (PvdA/VVD/D66) for temporal balance")
        return filtered
    else:
...
def main():
    ...
    print("-" * 40)
    coalition_edgelist_all = load_and_process_coalition_data()
    save_edgelist(coalition_edgelist_all, "coalition_edges.csv")
    
    # Create period-specific coalition edge lists
    coalition_edgelist_pre = filter_coalition_by_period(coalition_edgelist_all, period="pre")
    coalition_edgelist_post = filter_coalition_by_period(coalition_edgelist_all, period="post")
    print()
    ...
    # Study 1 versions (identical node structure - voting agreement only for QAP)
    ...
    # Use period-specific coalition data (pre excludes 2024 coalition)
    study2_coalition_pre = filter_edgelist_by_parties(coalition_edgelist_pre, study2_parties_pre, "Study 2 coalition pre")
    study2_coalition_post = filter_edgelist_by_parties(coalition_edgelist_post, study2_parties_post, "Study 2 coalition post")
    ...
    print()
    print("STUDY 1 (QAP) - Identical node structure, voting agreement only:")
    print("  • study1_edges_pre_election.csv - Voting agreements (pre-election)")
    print("  • study1_edges_post_formation.csv - Voting agreements (post-formation)")
    print()
    ...
    print("  • study2_coalition_*.csv - Coalition relationships (temporally filtered)")
    print("    └─ Pre-election: Excludes 2024 coalition (BBB/NSC/PVV/VVD)")
    print("    └─ Post-formation: Includes all historical coalitions")
    print()
