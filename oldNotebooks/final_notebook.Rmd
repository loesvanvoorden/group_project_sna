...
parties_to_remove <- c("Omtzigt","BIJ1", "BVNL", "50PLUS")
...
print("First 10 edge weights from CSV:")
print(head(edgelist_pre$weight, 10))
print(sprintf("Are they < 1? %s", all(edgelist_pre$weight <= 1)))
...
#extract adjacency matrix and make fully connected
...
## 4.2 Checking best cut-off threshold percentage pre

```{r study2-edge-attributes}
# Pre
total_motions_pre  <- length(unique(motion_data_pre$Besluit_Id))
counted_weights_pre <- igraph::E(g_study2_pre)$weight
igraph::E(g_study2_pre)$weight_count <- counted_weights_pre
igraph::E(g_study2_pre)$weight <- counted_weights_pre / total_motions_pre
snafun::g_summary(g_study2_pre)
edge_weights_pre <- igraph::E(g_study2_pre)$weight

# Post
total_motions_post  <- length(unique(motion_data_post$Besluit_Id))
counted_weights_post <- igraph::E(g_study2_post)$weight
igraph::E(g_study2_post)$weight_count <- counted_weights_post
igraph::E(g_study2_post)$weight <- counted_weights_post / total_motions_post
snafun::g_summary(g_study2_post)
edge_weights_post <- igraph::E(g_study2_post)$weight
...
# 4.3 Add edge attributes to Binary ERGM
```{r}

chosen_Cutoff <- "p75"

#Pre
thresholded_pre <- g_pre_list[[chosen_Cutoff]]

party_names_pre <- igraph::V(thresholded_pre)$name
...
#Add vertex attribute left_right
network::set.vertex.attribute(thresholded_pre_net, "left_right", igraph::V(thresholded_pre)$left_right)

# Adding coalition
edgelist_pre_coalition <- read.csv2("results/edge_lists/study2_coalition_filtered_pre.csv")
edgelist_pre_coalition_filtered <- subset(edgelist_pre_coalition,from %in% party_names & to %in% party_names)
g_pre_coa <- igraph::graph_from_data_frame(edgelist_pre_coalition_filtered,directed = FALSE,vertices = data.frame(name = party_names_pre))
coalition_pre_mat <- igraph::as_adjacency_matrix(g_pre_coa, attr = "weight",sparse = FALSE)
...
g_pre_cos <- igraph::graph_from_data_frame(edgelist_pre_cosponsor_filtered,FALSE,data.frame(name = party_names_pre))
cosponsor_pre_mat <- igraph::as_adjacency_matrix(g_pre_cos, attr = 'weight', sparse = FALSE)


# Post
thresholded_post <- g_post_list[[chosen_Cutoff]]

party_names_post <- igraph::V(thresholded_post)$name
...
# Adding coalition
edgelist_post_coalition <- read.csv2("results/edge_lists/study2_coalition_filtered_post.csv")
edgelist_post_cosponsor_filtered <- subset(edgelist_post_cosponsor,from %in% party_names & to %in% party_names)
g_post_coa <- igraph::graph_from_data_frame(edgelist_post_coalition_filtered,directed = FALSE,vertices = data.frame(name = party_names_post))
...
edgelist_post_cosponsor_filtered <- subset(edgelist_post_cosponsor,from %in% party_names & to %in% party_names)
g_post_cos <- igraph::graph_from_data_frame(edgelist_post_cosponsor,FALSE,data.frame(name = party_names_post))
...
ERGM (Exponential Random Graph Model) will be used in Study 2 to model network formation with covariates. ERGM allows us to test hypotheses about which factors influence voting agreement between parties.

## 6.2 Hypotheses and Model Terms

The ERGM will test the following hypotheses:
...
FinalModel <- ergm::ergm(
    ...
    kstar(3) + edgecov(cosponsor_post_mat) + 
